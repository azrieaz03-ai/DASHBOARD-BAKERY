...
class User(db.Model):
    ...
    Username = db.Column(db.String(45, collation="utf8mb4_bin"), unique=True, nullable=False)
    Password = db.Column(db.String(45, collation="utf8mb4_bin"), nullable=False)
...
def input_produksi_esp32():
    ...
    if not idRoti or not idUser or jumlah_produksi is None:
        return jsonify({"message": "Field idRoti, idUser, dan Jumlah_Produksi wajib diisi"}), 400

    # Parsing tanggal
    tanggal_produksi = datetime.fromisoformat(tanggal_input) if tanggal_input else waktu_wib()
    ...
    return jsonify({
        "message": f"Produksi {jumlah_produksi} roti berhasil disimpan",
        "total_terkini": jumlah_produksi
    }), 200
...
@app.route('/api/data_produksi')
def api_data_produksi():
    if 'role' not in session or session['role'] != 'Owner':
        return "", 401
        ...
        query = text("""
            SELECT
                ...
                COALESCE((
                    ...
                    ORDER BY rk2.Tanggal DESC, rk2.idRingkasan DESC
                    LIMIT 1
                ),0) + SUM(rk.Total_Produksi) - SUM(rk.Total_Terjual),
                SUM(rk.Total_Uang_Masuk)
            FROM ringkasan_baru rk
            JOIN produksi p ON rk.idProduksi = p.idProduksi
            JOIN roti r ON p.idRoti = r.idRoti
            WHERE DATE(rk.Tanggal) = :tanggal
...
@app.route('/api/data_produksi_kasir')
def api_data_produksi_kasir():
    if 'role' not in session or session['role'] != 'Kasir':
        return jsonify([])  # kosong kalau bukan kasir
